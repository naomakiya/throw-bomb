...
// ライトの構造体
cbuffer StateColor : register(b0)
{
    float4 Position;
    float4 Color;
    float4 time;
...
// 光の減衰パラメータ
static const float att0 = 0.05f; // 基本減衰
static const float att1 = 0.01f; // 線形減衰
static const float att2 = 0.02f; // 二次減衰
...
float4 main(PS_Input input) : SV_Target0
{
    ...
    return float4(1.0f, 0.0f, 0.0f, 1.0f); // **常に赤色を出力**
   ...
   // float maxY =  5.0f; 
   //  // 高さに基づいて補間値を計算
   // float factor = saturate((input.PositionWS.y - 1) / (3 - 1));
    
   //// 色を補間（オブジェクトの色を `Color.rgb` に近づける）
   // float3 finalColor = lerp(textureColor.rgb, Color.rgb, factor);
