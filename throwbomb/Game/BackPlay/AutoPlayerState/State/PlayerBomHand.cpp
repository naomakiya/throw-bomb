...
void PlayerBomHand::Render(const DirectX::SimpleMath::Matrix& view, const DirectX::SimpleMath::Matrix& projection)
{
	...
	// ワールド行列の計算
	Matrix faceWorld = Matrix::CreateScale(m_player->PLAYERMODLSCALE) * Matrix::CreateFromQuaternion(m_rotate * faceRotation) * Matrix::CreateTranslation(m_postion);
	Matrix bodyWorld = Matrix::CreateScale(m_player->PLAYERMODLSCALE) * Matrix::CreateFromQuaternion(m_rotate) * Matrix::CreateTranslation(m_postion);

	// 手のワールド行列を作成
	Matrix handL = Matrix::CreateScale(m_player->PLAYERMODLSCALE)
		* Matrix::CreateTranslation(m_postion + handOffsetL + Vector3(0.0f, m_hand, 0.0f)); // 左手の位置
	Matrix handR = Matrix::CreateScale(m_player->PLAYERMODLSCALE)
		* Matrix::CreateTranslation(m_postion + handOffsetR + Vector3(0.0f, m_hand, 0.0f)); // 右手の位置


	//足のワールド行列
	Matrix legL = Matrix::CreateScale(0.0085f) * Matrix::CreateFromQuaternion(m_rotate) * Matrix::CreateTranslation(m_postion + legOffsetL);
	Matrix legR = Matrix::CreateScale(0.0085f) * Matrix::CreateFromQuaternion(m_rotate) * Matrix::CreateTranslation(m_postion + legOffsetR);
...
void PlayerBomHand::Movement(const float& elapsedTime)
{
	...
	// プレイヤーの位置を更新する
	m_postion += velocity * elapsedTime;

	// プレイヤーの移動を試みる
	Vector3 newPosition = m_postion + velocity;

	// プレイヤーの位置を更新する
	m_postion = newPosition;
	// 座標を更新する
	if (kbState.Up)		m_postion += Vector3::Transform(Vector3(0.0f, 0.0f, 0.1f), m_rotate);
	if (kbState.Down)	m_postion -= Vector3::Transform(Vector3(0.0f, 0.0f, 0.1f), m_rotate);
}
