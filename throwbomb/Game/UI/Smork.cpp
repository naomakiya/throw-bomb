...
void Smork::LoadTexture(const wchar_t* path)
{
...
void Smork::Create(DX::DeviceResources* pDR)
{	
	m_pDR = pDR;
	...
	//	画像の読み込み
	LoadTexture(L"Resources/Textures/Smork.jpg");
...
void Smork::Update(float elapsedTime, DirectX::SimpleMath::Vector3 pos, float radius)
{
	m_timer += elapsedTime;
	DirectX::SimpleMath::Vector3 poss = pos;
	poss.y -= 0.5f;
	if (m_on)
	{
		CreateCircularParticles(8, 0.5f, 1.0f, 10.0f, 0.8f, poss);
		m_on = false;
	}
	for (auto it = m_starUtility.begin(); it != m_starUtility.end();)
	{
		if (!it->Update(elapsedTime , radius))
		{
...
void Smork::Render(DirectX::SimpleMath::Matrix view, DirectX::SimpleMath::Matrix proj)
{
	...
	SimpleMath::Vector3 SmorktDir = SimpleMath::Vector3::Forward;
	float dot = containedCameraDir.Dot(SmorktDir);
	float cross = SmorktDir.Cross(containedCameraDir).y;
	...
	for (SmorkUtility& li : m_starUtility)
	{
...
void Smork::CreateCircularParticles(int numParticles, float maxSpeed, float initialScale, float finalScale, float lifetime,DirectX::SimpleMath::Vector3 pos)
{
	float range = 10.0f;
