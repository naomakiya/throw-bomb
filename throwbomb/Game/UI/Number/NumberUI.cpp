...
// コンストラクタ
//---------------------------------------------------------
NumberUI::NumberUI(float score)
	: m_pDR(nullptr)
	, m_time{ score * 10.0f } // 100秒　１の位が少数第一
...
// テクスチャリソース読み込み
//---------------------------------------------------------
void NumberUI::LoadTexture(const wchar_t* path)
{
...
// 生成
//---------------------------------------------------------
void NumberUI::Create(DX::DeviceResources* pDR , bool isnumber)
{
	m_pDR = pDR;
...
void NumberUI::CreateShader5()
{
	...
	BinaryFile VSData = BinaryFile::LoadFile(L"Resources/Shaders/ParticleVS.cso");
	BinaryFile GSData = BinaryFile::LoadFile(L"Resources/Shaders/ParticleGS.cso");
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shaders/5NumberPS.cso");
...
// 描画
//---------------------------------------------------------
void NumberUI::Render(DirectX::SimpleMath::Vector3 pos)
{
	...
	{ 
		DirectX::VertexPositionColorTexture(DirectX::SimpleMath::Vector3(pos),
