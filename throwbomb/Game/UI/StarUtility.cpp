...
const static float RADIUS = 1.0f;  // 回転半径
...
bool StarUtility::Update(float elapsedTime)
{
	// 角度を時間経過に応じて増加させる
	float rotationSpeed = 2.25f;  // 回転速度（ラジアン/秒）
	...
	// 円運動を適用して座標を調整
	/*m_position.x = m_centerPosition.x + RADIUS * cos(DirectX::XMConvertToRadians(m_rotationAngle));
	m_position.z = m_centerPosition.z + RADIUS * sin(DirectX::XMConvertToRadians(m_rotationAngle));*/
	//	ライフを減らしていく
	m_life -= elapsedTime;
