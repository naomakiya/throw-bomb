...
{
    bool allEnemiesDefeated = true; // すべての敵が倒されたか判定するフラグ
    //更新
    for (auto& enemy : m_enemy)
    {
        //生きているなら
        if (enemy->GetExist())
        {
            ...
            allEnemiesDefeated = false; // 一体でも生存しているなら false にする
        }
    }
    ...
    // すべての敵が倒された場合、m_empty を true にする
...
void EnemyManager::StraighteningEnemyUpdate(const float& elapsedTime)
{
    ...
    // すべての突進する敵が倒された場合、m_empty を true にする
